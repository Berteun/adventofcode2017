...
def build_bridge_h(bridge, nodes):
    ...
    for (t,f) in nodes:
        ...
        #if bridge[0] == t:
        #    b = build_bridge_h([f, t] + bridge, nnodes)
        #    bridges.append(b)
        #if bridge[0] == f:
        #    b = build_bridge_h([t, f] + bridge, nnodes)
        #    bridges.append(b)
        if bridge[-1] == f:
        ...
        if bridge[-1] == t:
            ...
            bridges.append(b)
    cache[key] = max((sum(b), b) for b in bridges)[1]
    return cache[key]
...
def build_bridge(inp):
    ...
    return max((sum(b), b) for b in bridges)

def main():
    inp = read_input()
    print(build_bridge(inp)[0])
